...
// Function to delete from booking table
function deleteBooking($email) {
    global $conn;

    $stmt = $conn->prepare("DELETE FROM appointment WHERE email = ?");
    if ($stmt === false) {
        die('Prepare failed: ' . $conn->error);
    }
...
function deleteUserAccount($email) {
    ...
    // Delete from booking
    $bookingDeleted = deleteBooking($email);

    if ($aboutMeDeleted && $snapfeedDeleted && $userDeleted && $bookingDeleted) {
        return true;
    } else {
        return false;
    }
